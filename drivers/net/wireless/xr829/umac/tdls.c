				...
				struct cfg80211_chan_def *chandef)
{
	...
	local_bh_disable();
	__mac80211_subif_start_xmit(skb, dev, flags, 0);
	local_bh_enable();
...
void ieee80211_teardown_tdls_peers(struct ieee80211_sub_if_data *sdata)
{
	...
	list_for_each_entry_rcu(sta, &sdata->local->sta_list, list) {
		...
		mac80211_tdls_oper_request(&sdata->vif, sta->sta.addr,
				      ...
				      const u8 *peer, u16 reason)
{
	...
	rcu_read_lock();
	sta = mac80211_find_sta(&sdata->vif, peer);
	if (!sta || !sta->tdls) {
		rcu_read_unlock();
		return;
	}
	...
	mac80211_tdls_oper_request(&sdata->vif, peer,
