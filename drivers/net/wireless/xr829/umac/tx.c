...
void __mac80211_subif_start_xmit(struct sk_buff *skb,
				  struct net_device *dev,
				  u32 info_flags,
				  u32 ctrl_flags)
{
				       ...
				       struct net_device *dev)
{
	if (unlikely(ieee80211_multicast_to_unicast(skb, dev))) {
		...
		while ((skb = __skb_dequeue(&queue)))
			__mac80211_subif_start_xmit(skb, dev, 0, 0);
	} else {
		__mac80211_subif_start_xmit(skb, dev, 0, 0);
	}
			      ...
			      const u8 *dest, __be16 proto, bool unencrypted)
{
	...
	local_bh_disable();
	__mac80211_subif_start_xmit(skb, skb->dev, flags, ctrl_flags);
	local_bh_enable();
			      ...
			      const u8 *buf, size_t len)
{
	...
	local_bh_disable();
	__mac80211_subif_start_xmit(skb, skb->dev, 0,
